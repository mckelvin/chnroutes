...
def generate_ovpn(metric):
    ...
    print ("Usage: Append the content of the newly created routes.txt to your "
           "openvpn config file, and also add 'max-routes %d', which takes "
           "a line, to the head of the file." % (len(results)+20))
...
def fetch_ip_data():
    ...
    if not os.path.join(cache_dir):
        os.makedirs(cache_dir)
    apnic_cache_path = os.path.join(cache_dir, 'apnic-delegated-apnic-latest')
    if (not os.path.exists(apnic_cache_path) or
            time.time() - os.path.getmtime(apnic_cache_path) > ONE_DAY):
        print ("Fetching data from apnic.net, it might take a few minutes,"
               " please wait...")
        urllib.urlretrieve(url, apnic_cache_path)
